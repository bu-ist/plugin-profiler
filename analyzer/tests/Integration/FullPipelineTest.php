...
class FullPipelineTest extends TestCase
{
    ...
    public function testFullPipeline_NodesHaveRequiredFields(): void
    {
        ...
        foreach ($decoded['nodes'] as $node) {
            ...
            // Compound group nodes (namespace / dir) carry only id/label/type —
            // they are synthetic containers generated by JsonExporter, not entities
            // extracted from source files, so they intentionally have no file/line.
            if (in_array($data['type'], ['namespace', 'dir'], true)) {
                continue;
            }
